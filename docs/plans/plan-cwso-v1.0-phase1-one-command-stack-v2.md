...
- **Token budget:** 240k (v1: 200k — see "Changes from v1")
- **Supersedes:** [plan-cwso-v1.0-phase1-one-command-stack-v1.md](plan-cwso-v1.0-phase1-one-command-stack-v1.md)

## Changes from v1

- **Added C019** (sandbox trustworthiness for the non-KVM default path, P0, 40k),
  created by the human's approval decision 3 (2026-08-13): v1.0 mounts the user's
  repository read-write (C015) **conditional on** the sandbox tiering being trustworthy
  on the default non-KVM path. C015 now depends on C019's evidence artifact.
- CG1 exit criteria gained one row: C019's trustworthiness evidence exists and the
  read-write default cites it.
- Budget: 200k → 240k (roadmap total 1,360k → 1,400k, recorded in the roadmap's
  Approval section).
...
no orchestration semantics. Because the default mount becomes read-write (C015), the
degraded (non-KVM) sandbox path must carry evidence of trustworthiness (C019).

## Scope

- **In scope**: C010–C019 — compose profile removal, opt-in rollout profile, secret
  bootstrap, token script, feature-flag fold-in, user-repo mounting, `make up`,
  `cwso-doctor.sh`, the end-to-end smoke test that becomes the v1.0
  definition-of-done executable, and sandbox-tiering trustworthiness for the
  non-KVM default path.
- **Out of scope**: the filesystem projection (Phase 2); protocol work (Phase 3);
  ...
  with the compose file as changed here); promoting the Firecracker tier (v1.0 ships
  the fallback path, documented).
- **Assumptions**:
  ...
  - Sandbox degraded-mode detection already exists in `sandbox/router.go`; C017 surfaces it, C019 hardens and evidences it.
...
graph TD
  ...
  C010 --> C019[C019 Sandbox trustworthiness, non-KVM path]
  C019 --> C015[C015 Mount user repo read-write]
  C010 --> C015
...
C011–C015, C017 and C019 may run in parallel after C010 (compose-file edits serialize
per the risk below). C016 and C018 are sequential.
...
| C015 | Mount user repo read-write (CWSO_WORKSPACE_HOST) | devops-engineer | medium | [task-C015.md](../tasks/task-C015.md) |
...
| C019 | Sandbox trustworthiness, non-KVM default path | backend-developer | medium | [task-C019.md](../tasks/task-C019.md) |
...
C015 → deploy/docker-compose.yml volume mount (rw)  (consumed by: C016, C018, users)
...
C019 → docs/artifacts/sandbox-trustworthiness-v1.md + hardening (consumed by: C015, CG1, C061)
```
...
| Read-write workspace mount lets a bug corrupt the user's repo | Medium | High | Human decision 3 (2026-08-13): read-write ships **only with** C019's trustworthiness evidence for the non-KVM path; C015 cites that evidence; a C019 NO-GO escalates to the human before C015's default ships |
| C019 finds the degraded path cannot be made trustworthy | Medium | High | Explicit escalation rail in the C019 brief: stop, document, escalate `critical` — the read-write default then requires a human re-decision; silence is not an option |
...
- [x] CG0 closed (all Phase 0 exit criteria pass)
...
- [ ] C019's trustworthiness evidence (`docs/artifacts/sandbox-trustworthiness-v1.md`) exists and C015's read-write default cites it

## Approval

- [x] User approved on 2026-08-13 (incl. decision 3: read-write mount conditional on non-KVM sandbox trustworthiness → C019)
- [x] Plan locked; revisions create `plan-cwso-v1.0-phase1-one-command-stack-v3.md`

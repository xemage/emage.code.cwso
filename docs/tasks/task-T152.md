# Task T152 — v0.3.0 GA release readiness

- **Status:** in_review
- **Owner:** release-manager
- **Priority:** P0
- **Depends on:** T142, T147, T141
- **Based on:** `release-v0.3.0-rc1.md`, `checkpoint-012-nextgen-ga-prep.md`

## Objective

Promote `v0.3.0-rc1` to **v0.3.0 GA** after post-RC hardening. Publish release artifact,
CHANGELOG, GitLab release, and checkpoint.

## Acceptance Criteria

- [x] `develop` CI green on release commit
- [x] `docs/artifacts/release-v0.3.0.md` with scope vs RC delta
- [x] CHANGELOG section for v0.3.0
- [ ] Annotated tag `v0.3.0` on `develop`
- [ ] GitLab release published
- [x] `checkpoint-013-v0.3.0-ga.md` written
- [ ] Task board marks T152 done; plan updated

## Notes

Stakeholder RC sign-off external; GA proceeds CONDITIONAL_PASS per release artifact.

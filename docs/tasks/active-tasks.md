...
Merged @ `8aa5c74` (MR !58). `deploy:registry` → `needs:optional` on `e2e:phase2`.
...
### T145 (done) — Rollout num_samples session fan-out

Merged @ MR !59. `num_samples` (1–32), per-session callbacks, integration test N=3.
...
### T149 (done) — Trajectory builder Polar parity

Merged @ MR !65 → `develop` (`38ac3f5`). `per_request` + `prefix_merge` strategies, EOT
interstitial masking (`loss_mask=0`), partition-key chain splitting for sub-agents/compaction,
per-task `trajectory_builder_strategy` on submit. Feature-flagged
`CWSO_ROLLOUT_TRAJECTORY_BUILDER_ENABLED` (default off). Golden tests: Polar Fig. 4 fixture.

Brief: `task-T149.md`.

### T156 (done) — Comprehensive installation guide v2

Merged @ MR !64 → `develop` (`71b0d17`). `docs/user/installation-v2.md` — full v0.4.0 adoption
doc (architecture, flags, workflows). Trajectory builder env vars documented in follow-up board sync.
...
### Polar parity backlog (T150–T151)

Gap analysis: `docs/artifacts/polar-gap-analysis-v1.md`. T149 merged; T144–T148/T154–T156 done.
Post-GA optional: differential prompting (T150), offline SFT (T151).

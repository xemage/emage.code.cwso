...
type Config struct {
	...
	RolloutGatewayStagingEnabled         bool     // enable INIT/READY/RUNNING/POSTRUN gateway pools (T146)
	RolloutGatewayInitWorkers            int      // INIT pool worker count (T146)
	RolloutGatewayReadyBuffer            int      // READY buffer between INIT and RUNNING (T146)
	RolloutGatewayRunningWorkers         int      // RUNNING pool worker count (T146)
	RolloutGatewayPostRunWorkers         int      // POSTRUN pool worker count (T146)
	RolloutGatewaySessionTimeout         int      // per-session RUNNING timeout seconds (T146)
	RolloutEvaluatorPrewarmEnabled       bool     // start evaluator prewarm during RUNNING (T146)
	RolloutEvaluatorRegistryEnabled      bool     // enable post-run evaluator registry (T148)
...
func Load(_ string) (*Config, error) {
	...
	c := &Config{
		...
		RolloutGatewayStagingEnabled:         envBool("CWSO_ROLLOUT_GATEWAY_STAGING_ENABLED", false),
		RolloutGatewayInitWorkers:            envInt("CWSO_ROLLOUT_GATEWAY_INIT_WORKERS", 2),
		RolloutGatewayReadyBuffer:            envInt("CWSO_ROLLOUT_GATEWAY_READY_BUFFER", 4),
		RolloutGatewayRunningWorkers:         envInt("CWSO_ROLLOUT_GATEWAY_RUNNING_WORKERS", 4),
		RolloutGatewayPostRunWorkers:         envInt("CWSO_ROLLOUT_GATEWAY_POSTRUN_WORKERS", 2),
		RolloutGatewaySessionTimeout:         envInt("CWSO_ROLLOUT_GATEWAY_SESSION_TIMEOUT_SECONDS", 300),
		RolloutEvaluatorPrewarmEnabled:       envBool("CWSO_ROLLOUT_EVALUATOR_PREWARM_ENABLED", false),
		RolloutEvaluatorRegistryEnabled:      envBool("CWSO_ROLLOUT_EVALUATOR_REGISTRY_ENABLED", false),
	...
	if c.RolloutGatewayStagingEnabled {
		if c.RolloutGatewayInitWorkers <= 0 {
			return nil, fmt.Errorf("CWSO_ROLLOUT_GATEWAY_INIT_WORKERS must be > 0 when gateway staging is enabled")
		}
		if c.RolloutGatewayReadyBuffer <= 0 {
			return nil, fmt.Errorf("CWSO_ROLLOUT_GATEWAY_READY_BUFFER must be > 0 when gateway staging is enabled")
		}
		if c.RolloutGatewayRunningWorkers <= 0 {
			return nil, fmt.Errorf("CWSO_ROLLOUT_GATEWAY_RUNNING_WORKERS must be > 0 when gateway staging is enabled")
		}
		if c.RolloutGatewayPostRunWorkers <= 0 {
			return nil, fmt.Errorf("CWSO_ROLLOUT_GATEWAY_POSTRUN_WORKERS must be > 0 when gateway staging is enabled")
		}
		if c.RolloutGatewaySessionTimeout <= 0 {
			return nil, fmt.Errorf("CWSO_ROLLOUT_GATEWAY_SESSION_TIMEOUT_SECONDS must be > 0 when gateway staging is enabled")
		}
	}
